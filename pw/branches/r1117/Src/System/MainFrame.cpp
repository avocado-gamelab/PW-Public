...
void NMainFrame::ResizeWindow( unsigned long width, unsigned long height, bool isFullScreen )
{
  ...
  // gfx_borderless: окно без рамки во весь монитор, без WS_EX_TOPMOST.
  // В отличие от exclusive-fullscreen не захватывает видеорежим — Alt-Tab мгновенный,
  // Discord-оверлей и скриншоты работают, второй монитор не чернеет.
  Render::RenderMode currentMode;
  Render::GetRenderModeFromConfig( currentMode );
  const bool isBorderless = !isFullScreen && currentMode.isBorderless;
  ...
  {
    // WS_POPUP без WS_EX_TOPMOST + полный rcMonitor (не rcWork — чтобы перекрыть
    // таскбар, иначе снизу видна его полоса и курсор не доходит до верхнего края,
    // edge-pan камеры вверх не триггерится).
    RECT currentWindowRect;
    ::GetWindowRect( hWindow, &currentWindowRect );
    HMONITOR hMon = ::MonitorFromRect( &currentWindowRect, MONITOR_DEFAULTTOPRIMARY );
    MONITORINFO mi;
    mi.cbSize = sizeof(mi);
    if( hMon != NULL && ::GetMonitorInfo( hMon, &mi ) )
      rect = mi.rcMonitor;
    else
      rect = GetMonitorRect( currentWindowRect );

    dwStyle = ( dwStyle & ~dwStyleNonFullscreen ) | WS_POPUP;
