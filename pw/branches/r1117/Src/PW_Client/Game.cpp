...
int __stdcall PseudoWinMain( HINSTANCE hInstance, HWND hWnd, LPTSTR lpCmdLine, SPluginSettings * pluginSett )
{
  ...
  if ( paramsArg && paramsArg[0] != '\0' )
  {
    ...
    {
      WarningTrace( "-params present but invalid (base64Ok=%d jsonOk=%d) — falling back to default server address from cfg",
        (int)base64Ok, (int)jsonOk );
    }
  }
  else
  {
    WarningTrace( "-params argument missing — falling back to default server address from cfg (legacy path)" );
  }
